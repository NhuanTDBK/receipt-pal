...
# Builtins allowed inside generated query code.
_SAFE_BUILTINS: dict = {
    "len": len,
    "sum": sum,
    "min": min,
...
# Keywords that indicate write operations — block them before exec.
_WRITE_GUARDS = (
    "session.add",
    "session.delete",
    "session.commit",
...
def run_query(
    ...
    namespace = {
        ...
        # Immutable user scope — model cannot override this
        "user_id": user_id,
    }
    ...
    with ctx.context.get_session() as session:
        ...
        local_ns: dict = {}
        try:
            exec(query_code, namespace, local_ns)  # noqa: S102 — PoC controlled exec
        except Exception:
